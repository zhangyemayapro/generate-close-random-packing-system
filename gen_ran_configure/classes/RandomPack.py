...
class RandomPack:
    ...
    def set_log(self, logname="out.log"):
        ...
        self.writeLogValue("tot_step\t phi\t maxoverlap \t q6lmean\t tol1 \t tol2 \t changeRflag")
    ...
    def logging(self):
        self.log.flush()
        self.writeLogValue("%.10d\t"%self.tot_step
                           +"%.3f\t"%self.phi
                           +"%.3f\t"%self.maxoverlap
                           +"%.3f\t"%self.q6lmean
                           +"%.3f\t"%self.tol1
                           +"%.3f\t"%self.tol2
                           +"changeRflag\t"+str(self.changeRflag))
    ...
    def showInjavis(self):
        ...
        deprecated.dump.pos(filename="../data/dump.pos", period=1)
        hoomd.run(1)
    ...
    # def dumpPosGsd(self,filename = "changeR.gsd"):
        ...
        mc.shape_param.set("A", diameter=2 * self.radii)
        hoomd.dump.gsd(
            filename=filename, group=hoomd.group.all(), dynamic=['attribute','property'],truncate=False, period=1
        )
